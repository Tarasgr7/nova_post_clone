...
async def create_shipment(
  ...
  if shipment_data.receiver_id == user.get("id"):
    raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="You can't send your own shipment")
  ...
  shipment=Shipment(
    ...
    price=shipment_data.price,  # price will be calculated later
  )
...
@router.get('/from_me',status_code=status.HTTP_200_OK)
async def get_my_shipments(db: db_dependency, user: user_dependency):
  if not user:
    raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Authentication required")
  shipments = db.query(Shipment).filter(Shipment.sender_id == user.get("id")).all()
  return shipments
...
async def get_my_shipments(db: db_dependency, user: user_dependency):
  if not user:
    raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Authentication required")
  shipments = db.query(Shipment).filter(Shipment.receiver_id == user.get("id")).all()
  return shipments
...
async def change_status(tracking_number: str, shipment_update: ShipmentUpdate, db: db_dependency, user: user_dependency):
  if not user:
    raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Authentication required")
  if existing_status(shipment_update.status) != True:
    raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid status")
  shipment = db.query(Shipment).filter(Shipment.tracking_number == tracking_number).first()
  if not shipment:
    raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Замовлення не знайдено")
  if  user.get("role") not in ['admin','courier']:
    raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Тільки адміністратори та курери можуть змінювати статус замовлення")
...
async def delete_shipment(tracking_number: str, db: db_dependency, user: user_dependency):
  ...
  if user.get("id") != shipment.sender_id:
    raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Тільки відправник замовлення може видаляти його")
  db.delete(shipment)
  db.commit()
