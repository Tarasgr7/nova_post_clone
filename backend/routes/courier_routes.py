...
async def create_courier(courier_data: CourierCreate, db:db_dependency):
  ...
  courier=Courier(
    ...
    vehicle_id=courier_data.vehicle_id,
    active=courier_data.active,
    locate=courier_data.locate
  )
  user=db.query(User).filter(User.id == courier_data.user_id).first()
  if not user:
